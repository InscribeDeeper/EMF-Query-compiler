...
MFVector = defaultdict(lambda: {'cust':'', 'prod':'', 'x_count_quant':0, 'x_sum_quant':0.0, 'x_avg_quant':0.0, 'y_count_quant':0, 'y_sum_quant':0.0, 'y_avg_quant':0.0, 'x_count_prod':0, 'x_sum_quant':0.0, 'y_count_prod':0})
MF_idx=0
MFMap = {}
...
for gv in ['x', 'y']:
	if gv == 'x':
		for i in range(len(records)):
			...
			if ((tmprec['cust']==MFVector[index]['cust'])):
	...
	if gv == 'y':
		for i in range(len(records)):
			...
			if ((tmprec['cust']==MFVector[index]['cust']) and (tmprec['prod']!=MFVector[index]['prod'])):
				...
				MFVector[index]['y_count_prod']+=1


# Outputing
print('='*100)
print('{:<6}|{:>15}|{:>15}|{:>15}|{:>15}|{:>15}|{:>15}|{:>15}|'.format('index','cust','prod','x_count_prod','x_avg_quant','y_avg_quant','x_sum_quant','y_count_prod'))
print('='*100)
# Having
output_idx=0
for index in range(len(MFVector)):
	if MFVector[index]['x_count_prod']>0 and MFVector[index]['y_count_prod']>1:
		output_idx+=1
		...
		print('{:>15.0f}|'.format(MFVector[index]['y_count_prod']), end='')
		print('')
